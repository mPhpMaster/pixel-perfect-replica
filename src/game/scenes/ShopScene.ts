...
export class ShopScene extends Phaser.Scene {
    ...
    create(): void {
        ...
        // Title
        const title = this.add.text(width / 2, 40, 'SHOP', {
        ...
        // Coin display
        this.coinText = this.add.text(width / 2, 80, `COINS: ${this.meta.coins} 💰`, {
            fontFamily: '"Press Start 2P"',
            fontSize: '14px',
            color: '#ffdd00',
        });
        ...
        // Back button
        this.createButton((width / 2) - 216, height - 60, 'BACK TO MENU', () => {
            this.scene.start('MainMenuScene');
        });

        // Reset button
        this.createButton((width / 2), height - 60, 'RESET', () => {
            saveMeta(getDefaultMeta({coins: this.meta.coins}));
            this.refreshUI();
        ...
        this.input.keyboard!.on('keydown-F4', (event) => {
            ...
            saveMeta(getDefaultMeta({coins: 100000}));
            this.refreshUI();
        });
    }
    ...
    private createUpgradeItem(x: number, y: number, upgrade: typeof META_UPGRADES[0]): Phaser.GameObjects.Container {
        ...
        const value = upgrade.effect(level);
